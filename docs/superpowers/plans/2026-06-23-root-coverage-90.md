...
The fastest, safest needle-mover: **+22 Query roots** (14 → 36/57), all request/response, all reusing `base.py`. Count breakdown (schema-verified): metrics 1 + UPS 3 + plugins 4 + logs 2 + OIDC 2 + system.py 8 (`network`, `cloud`, `services`, `display`, `settings`, `systemTime`, `timeZoneOptions`, `vars`) + disks 1 (`assignableDisks`) + rclone 1 = **22**.

- **New `tools/metrics.py`** — `unraid_get_metrics` (Query `metrics`; fold CPU+memory+temperature into one tool). Models: `Metrics`, `CpuUtilization`, `MemoryUtilization`, `TemperatureMetrics`, `TemperatureSensor`, `TemperatureSummary` (schema-verified names; `Metrics implements Node` at L2597 with **`cpu`/`memory`/`temperature` all nullable** → model fields `Optional`). **Select `cpu.percentTotal`+`cpu.cpus`(load only), all memory scalars, `temperature.summary`+`temperature.sensors` but OMIT `sensors[].history`** (verified unbounded `[TemperatureReading!]` at L2552; belongs to the stream). BigInt memory → `str`.
...
**Registry edits (schema-verified against the current `INTENTIONALLY_UNCOVERED['Query']`):** promoting these 22 empties four groups completely — **delete them outright** (an empty `frozenset()` passes the tests but is dead, misleading code): `"metrics & logs …"` (all of `metrics`/`logFile`/`logFiles`), `"network & cloud …"` (`network`/`cloud`), `"plugins …"` (all 4), `"UPS …"` (all 3). **Trim** three groups: `"OIDC / SSO"` → drop `isSSOEnabled`/`publicOidcProviders`, keep `oidcProviders`/`oidcProvider`/`oidcConfiguration`/`validateOidcSession` (sharpened secret-leak reason — `OidcProvider.clientSecret` at snapshot L2711); `"config, settings & display"` → drop `display`/`settings`/`systemTime`/`timeZoneOptions`/`vars`, keep `config`/`customization`/`publicTheme`/`isFreshInstall`; `"identity & multi-server inventory"` → drop `services`/`assignableDisks`/`rclone`, keep `online`/`owner`/`internalBootContext`/`server`/`servers`. Each promoted field must leave the registry the same PR it gains a tool (`test_registry_has_no_stale_entries` fails on "now covered").
...
- **Keep declined:** `archiveAll` (superseded by `archiveNotifications`, which `unraid_archive_all_notifications` already invokes per migration #176 — do NOT add a separate tool; leave declined in registry), `recalculateOverview` (internal housekeeping; keep declined, reason=internal).
...
- **Extend `tools/system.py`**: `unraid_update_temperature_config` (`updateTemperatureConfig`; typed `TemperatureConfigInput`), `unraid_update_server_identity` (`updateServerIdentity`; flat `name`/`comment`/`sysModel`), `unraid_update_system_time` (`updateSystemTime`; **`UpdateSystemTimeInput` has 4 fields — `timeZone`, `useNtp`, `ntpServers`, and `manualDateTime: String` (YYYY-MM-DD HH:mm:ss, used when NTP disabled)** — verified at snapshot L3532–3548; include all four as optional params, build the input omitting unset keys). Pairs with the Phase-1 `unraid_get_system_time` / `unraid_list_timezone_options` reads.
...
**One corrected analyst datum:** `UpdateSystemTimeInput` has a **4th field, `manualDateTime: String`** (manual time when NTP is off), which two analysts truncated — confirmed at snapshot L3532–3548. Include it in `unraid_update_system_time`.
